...
pushd linuxdeploy-plugin-qt-examples/QtQuickControls2Application
    ...
    pushd build
        ...
        "$linuxdeploy_bin" -v0 --appdir "$PWD"/AppDir --plugin qt --output appimage
...
pushd linuxdeploy-plugin-qt-examples/QtWidgetsApplication
    ...
    pushd build-platforms
        ...
        env OUTPUT=platforms.AppImage "$linuxdeploy_bin" -v0 --appdir "$PWD"/AppDir --plugin qt --output appimage
